...
import { requirements, WA_LINK } from "@/lib/data";
    ...
    <section className="py-20 px-4 transition-colors duration-300" style={{ background: "var(--rp-primary-bg)" }}>
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          >
            Syarat Awal Pengajuan KPR
          </h2>
          <p style={{ color: "var(--rp-gray-text)" }}>
            Siapkan dokumen berikut. Tim kami bantu lengkapi saat konsultasi.
          </p>
        </motion.div>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            {requirements.map((req, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: "var(--rp-primary)" }}>
                  <FileCheck size={12} color="white" />
                </div>
                <span className="text-sm" style={{ color: "var(--rp-dark)" }}>{req}</span>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mt-8">
            ...
            >
              ...
              Konsultasi Syarat via WhatsApp
            </WhatsAppLink>
          </div>
        </motion.div>
      </div>
